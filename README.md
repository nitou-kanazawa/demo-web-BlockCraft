...
| インベントリ | E |
| モブ攻撃 | 左クリック（ブタ/ヒツジ。倒すと豚肉/羊毛をドロップ） |
| ミュート | M |
| 解除 | Esc |
...
- **動物モブ** — ブタとヒツジが草地にスポーンし徘徊（段差はジャンプで越える）。攻撃するとノックバック・被弾フラッシュ、倒すと豚肉/羊毛をドロップ
- **昼夜サイクル** — 1日4分。太陽の運行に合わせて空・霧・光量が変化し、朝焼け/夕焼けも
- **効果音** — WebAudio でリアルタイム合成（素材別の採掘/破壊/設置音、ジャンプ、クラフト等。音声ファイル不使用）
- **パーティクル** — 採掘チップ・破壊バースト・設置パフ・被弾/撃破エフェクト
...
│   ├── mobs.js      # 動物の徘徊AI・スポーン管理・戦闘
│   ├── daynight.js  # 昼夜サイクル（太陽・空色・光量）
│   ├── particles.js # パーティクルシミュレーション
│   ├── soundDefs.js # 効果音のシンセパラメータ定義
│   └── blockColors.js # ブロック代表色（UI/パーティクル共有）
├── render/        # Three.js 依存
│   ├── atlas.js         # プロシージャルテクスチャアトラス
│   ├── worldRenderer.js # チャンクメッシュの構築 / 破棄管理
│   ├── mobRenderer.js   # 動物のボックスモデル・歩行アニメ・被弾フラッシュ
│   └── particleRenderer.js # パーティクルの Points 描画
├── audio/
│   └── soundPlayer.js # WebAudio シンセ（効果音再生）
├── player/
...
| 12 | 昼夜サイクル（太陽・空色・光量） | #12 | ✅ |
| 13 | SE対応（WebAudioプロシージャル効果音） | #13 | ✅ |
| 14 | パーティクル（採掘チップ・破壊・設置） | #14 | ✅ |
| 15 | モブへの攻撃（ダメージ・ノックバック・ドロップ） | #15 | ✅ |

各 PR にはユニットテスト（Vitest, 計150件）とヘッドレス Chromium での動作確認を含みます。
